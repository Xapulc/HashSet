...
private:
    ...
    public:
        L1ListLinker* next;
        L1ListLinker(): next(0) {}
        L1ListLinker(const L1ListLinker& h): next(h.next) {}
        void link(const L1ListLinker* h) { next = (L1ListLinker*) h; }
    };

    class L1ListElement: public L1ListLinker {
    public:
        Pair pair;
        L1ListElement():
            L1ListLinker(),
    ...
    int hashTableSize;
    L1ListLinker* hashTable;
    int numElements;

public:
    HashSet():
        hashTableSize(1021),    // Prime number
        hashTable(new L1ListLinker[hashTableSize]),
        numElements(0)
    {}
...
private:
    ...
    L1ListLinker* search(const HashSetKey* key) const;
};
