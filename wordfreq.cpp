...
int main(int argc, char *argv[]) {
    ...
    if (isGetPopular) {
        int maxWords = 20;
        int *countWords = new int[20];
        countWords[0] = -1;   //  undefined
        int curWords = 0;
        ...
        while (i != e) {
            ...
            if (curWords == 0) {
                ...
                for (int j = 0; j < curWords; j++) {
                    if (number >= countWords[j]) {
                        int buf1, buf2 = countWords[j];
                        countWords[j] = number;

                        for (int k = j + 1; k < curWords - 1; k++) { //  shift
                            buf1 = countWords[k];
                            countWords[k] = buf2;
                            buf2 = buf1;
                        }
                        ...
                        if (curWords < maxWords) {
                            buf1 = countWords[curWords-1];
                            countWords[curWords-1] = buf2;
                            countWords[curWords] = buf1;
                            curWords++;
                        } else {
                            countWords[curWords-1] = buf2;
                        }

                        break; //  we add value in sorted massive
                    }
                }
            }
            ++i;
        ...
        HashSet::const_iterator i = set.begin();
        HashSet::const_iterator e = set.end();
        printf("The text contains the following 20 the most popular words:\n");

        while (i != e) {
            const HashSet::Pair &pair = *i;
            const Word *w = (const Word *) pair.key;
            int number = ((const Integer *) pair.value)->number;
            if (number >= countWords[curWords-1])
                printf(
                        "%d\t%s\n", number, w->getString()
                );
            ++i;
